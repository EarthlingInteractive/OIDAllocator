...
class EarthIT_OIDAllocator_PageAction_ShowDataTable extends EarthIT_OIDAllocator_PageAction_TemplatePageAction
{
	protected $rc;
	protected $items;
	
	public static function getInstance( EarthIT_OIDAllocator_Registry $reg, EarthIT_Schema_ResourceClass $rc, $items=null ) {
		$collectionName =
			$rc->getFirstPropertyValue(EarthIT_CMIPREST_NS::COLLECTION_NAME) ?:
			EarthIT_Schema_WordUtil::pluralize($rc->getName());
		
		$className = 'EarthIT_OIDAllocator_PageAction_Show'.EarthIT_Schema_WordUtil::toPascalCase($collectionName);
		if( !class_exists($className) or !is_subclass_of($className, 'EarthIT_OIDAllocator_PageAction_ShowDataTable') ) {
			$className = 'EarthIT_OIDAllocator_PageAction_ShowDataTable';
		}
	 ...
	 * template responses)
	 */
	public function __construct( EarthIT_OIDAllocator_Registry $reg, EarthIT_Schema_ResourceClass $rc, $items=null ) {
