...
class EarthIT_OIDAllocator_PageAction_FileUpload extends EarthIT_OIDAllocator_PageAction
{
	protected $req;
	
	public function __construct( EarthIT_OIDAllocator_Registry $reg, EarthIT_OIDAllocator_Request $req ) {
		...
		if( $this->isMultipart() ) {
			foreach( $this->getFiles() as $k=>$f ) {
				try {
					...
					$blobId = EarthIT_OIDAllocator_BlobIDUtil::urnToBasename($urn);
